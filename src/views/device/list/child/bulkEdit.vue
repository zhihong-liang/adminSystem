  ...
  <CnDialog ref="bulkEditRef" v-bind="dialogProps">
    ...
    <div v-if="step1 === 3">
      <CnForm ref="stpeThreeBulkEditFromRef" v-bind="stpeThreeBulkEditFrom">
        <template #timeSlot>
          <BusinessHours v-model="businessHoursWeek" @change="businessChange" />
        </template>
      </CnForm>
    </div>
...
import { devBaseInfoList, devGroupList, devBaseInfoEditList } from '@/api/device'
...
onMounted(async () => {
  matList.value = await mattersProgrammeListPageUtils().then((res) => {
    return res
  })
  open()
})
